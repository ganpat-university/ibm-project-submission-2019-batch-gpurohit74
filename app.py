...
def socialMediaInsights():
    ...
    return render_template('SocialMediaInsights.html', followers=followers,friendList=friendList,maxSent=maxSent,minSent=minSent,avgSent=avgSent,totSent=totSent,mutedIdsCount=mutedIdsCount,tables=[df1.to_html(classes='data')])
...
def success():
    if request.method == 'POST':
        ...
        if not allowed_file(f.filename):
            ...
            percentage_retention=round((df['Recurrent Costumer'].loc[df['Recurrent Costumer']=='Yes'].count()/df['Recurrent Costumer'].count()),2)
            #Males:Females Ratio
        ...
        return render_template("CompanyInsights2.html",pageLink=pageLink, name = name,total_data_points = total_data_points,total_features = total_features,percentage_retention = percentage_retention,males_females = males_females,tables=[df1.to_html(classes='data')])
